...
$page_data = [
    'title' => $pageTitle . ' | SanguineIT Articles',
    'description' => !empty($post['excerpt']) ? $post['excerpt'] : $pageTitle,
    'canonical' => sit_base_url() . '/article-single.php?slug=' . rawurlencode($slug),
    'og_image' => !empty($post['featured_image']) ? $post['featured_image'] : '',
    'json_ld' => [sit_article_schema($post, $slug, 'article-single.php', 'Article')],
];
