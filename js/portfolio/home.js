...
function sectionTeamCarousselPrg() {
    ...
    const $progress = $('#we-lead .wrapper-img .container-action .btn .progress');
    $progress.removeClass('active');
    setTimeout(() => {
        $progress.addClass('active');
    }, 50);

    const $elements = $('#we-lead .wrapper-img .container-el .el');
    const numberEl = $elements.length;

    // Hide current element
    $elements.removeClass('show');
    $elements.fadeOut(700);

    // Increment and wrap around countEl
    countEl = (countEl + 1) % numberEl;

    // Show next element
    setTimeout(() => {
        $elements.eq(countEl).fadeIn(0).addClass('show');
    }, 700);
}

// Start carousel on DOM ready
$(document).ready(function () {
    const $elements = $('#we-lead .wrapper-img .container-el .el');
    $elements.hide().eq(0).show().addClass('show');

    intervalTeamCaroussel = setInterval(sectionTeamCarousselPrg, delayTeamCaroussel);
});
...
$(function() {
    ...
    var scene = new ScrollMagic.Scene({
        ...
        TweenLite.to(".appi-ins-portfolio-sec", 1, {
            ease: Power4.easeInOut,
            css: {
                backgroundColor: "#d455b9"
            }
        });
        TweenLite.to(".appi-portfolio-sec-left, .clockbg-dial", 1, {
            ease: Power4.easeInOut,
            css: {
                backgroundColor: "#d42cb1"
            }
        });
        TweenLite.to(".appi-portfolio-sec-left figure.two", 1, {
            ease: Power4.easeInOut,
